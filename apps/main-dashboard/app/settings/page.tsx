...
export default function SettingsPage() {
  ...
  const getLastUsed = async (keyId: string) => {
    try {
      const token = await getToken();
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_SERVER_URI}/api-keys/${keyId}`,
        {
          headers: {
            Authorization: `Bearer ${token}`,
          },
  ...
  const cooldownOver =
    apiKeys && apiKeys[0]?.created_at
      ? now - new Date(apiKeys[0].created_at).getTime() > 5 * 60 * 1000
      : true;
            ...
            {apiKeys?.length !== 0 && !isLoading ? (
              <>
                {apiKeys?.map((apiKey: any) => (
                  <div
                    key={apiKey?.id}
                    className="bg-gray-100 dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-md px-4 py-3"
                  >
                      ...
                      <div>
                        ...
                        <p className="mt-1 text-sm dark:text-gray-200">
                          ...
                          <span className="dark:text-white">
                            {lastUsed ? format(lastUsed) : "Not use yet!"}
                          </span>
                        </p>
                      </div>
                        ...
                        <div className="flex gap-2">
                          {cooldownOver ? (
                            <button
                              ...
                              onClick={() =>
                                handleReGenerateSecretKey(apiKey?.id)
                              }
                            >
                              Regenerate Key
                            </button>
